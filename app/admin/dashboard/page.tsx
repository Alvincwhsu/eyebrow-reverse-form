...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FAFBF9] text-[#2D3B2E] font-sans pb-12">
      {/* 導覽列 */}
      <header className="bg-white border-b border-[#E2EBE2] px-6 py-4 flex justify-between items-center sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-3">
          <span className="text-xl font-bold text-[#2D3B2E]">預約管理後台</span>
          <span className="text-xs font-semibold bg-[#E8F0E8] text-[#5B7B5E] px-2.5 py-1 rounded-full">
            總預約量: {bookings.length} 筆
          </span>
        </div>
        <div className="flex items-center gap-4">
          <div className="flex bg-[#F0F5F0] p-1 rounded-xl text-xs font-medium">
            <button
              onClick={() => setTab('orders')}
              className={`px-4 py-2 rounded-lg transition-all ${tab === 'orders' ? 'bg-[#5B7B5E] text-white shadow-sm' : 'text-[#687869]'}`}
            >
              📋 訂單管理
            </button>
            <button
              onClick={() => setTab('analytics')}
              className={`px-4 py-2 rounded-lg transition-all ${tab === 'analytics' ? 'bg-[#5B7B5E] text-white shadow-sm' : 'text-[#687869]'}`}
            >
              📊 客戶數據統計
            </button>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 pt-8">
        ...
        {tab === 'orders' && (
          <div className="space-y-6">
            <div className="bg-white rounded-2xl border border-[#E2EBE2] shadow-sm overflow-hidden">
              <table className="w-full text-left text-xs">
                <thead className="bg-[#FAFBF9] border-b border-[#E2EBE2] text-[#5B7B5E] font-semibold">
                  <tr>
                    <th className="p-4">顧客姓名 / 電話</th>
                    <th className="p-4">預約項目 / 費用</th>
                    <th className="p-4">意願時間順位</th>
                    <th className="p-4">最終確定時間</th>
                    <th className="p-4">內部備註 (僅店家可見)</th>
                ...
                <tbody className="divide-y divide-[#E2EBE2]">
                  {bookings.map((b) => (
                    <tr key={b.id} className="hover:bg-[#F9FBF9] transition-all">
                      ...
                      <td className="p-4">
                        {b.confirmed_date ? (
                          <span className="font-bold text-[#2D3B2E] bg-[#E8F0E8] px-2.5 py-1 rounded-md border border-[#D8E5D9]">
                            {b.confirmed_date} {b.confirmed_time_slot}
                          </span>
                        ) : (
                          <span className="text-amber-600 font-medium bg-amber-50 px-2 py-0.5 rounded">尚未確認時間</span>
                        )}
                      </td>
                      ...
                      <td className="p-4">
                        {b.status === 'confirmed' && <span className="text-emerald-700 bg-emerald-100 px-2.5 py-1 rounded-full font-bold">已確認預約</span>}
                        {b.status === 'pending' && <span className="text-amber-700 bg-amber-100 px-2.5 py-1 rounded-full font-bold">待確認</span>}
                        {b.status === 'cancelled' && <span className="text-gray-500 bg-gray-100 px-2.5 py-1 rounded-full font-bold">已取消</span>}
                      </td>
                      <td className="p-4 space-x-2">
                        <button
                          onClick={() => openModal(b)}
                          className="px-3 py-1.5 bg-[#5B7B5E] text-white rounded-lg hover:bg-[#4A664D] transition-all font-medium"
                        >
                          審核 / 編輯
                        </button>
                      </td>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white p-6 rounded-2xl border border-[#E2EBE2] shadow-sm">
                ...
                <p className="text-3xl font-extrabold text-[#5B7B5E] mt-2">NT$ {totalRevenue.toLocaleString()}</p>
              </div>
              <div className="bg-white p-6 rounded-2xl border border-[#E2EBE2] shadow-sm">
                <p className="text-xs font-semibold text-[#7A8A7B]">總預約筆數</p>
                <p className="text-3xl font-extrabold text-[#2D3B2E] mt-2">{bookings.length} 筆</p>
              </div>
              <div className="bg-white p-6 rounded-2xl border border-[#E2EBE2] shadow-sm">
                <p className="text-xs font-semibold text-[#7A8A7B]">當月壽星佔比</p>
                <p className="text-3xl font-extrabold text-[#2D3B2E] mt-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white p-6 rounded-2xl border border-[#E2EBE2] shadow-sm space-y-3">
        ...
        {selectedBooking && (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-3xl max-w-xl w-full p-6 space-y-6 max-h-[90vh] overflow-y-auto shadow-xl">
              <div className="flex justify-between items-center border-b border-[#E2EBE2] pb-3">
                <h2 className="text-lg font-bold text-[#2D3B2E]">審核預約與設定</h2>
                <button onClick={() => setSelectedBooking(null)} className="text-gray-400 hover:text-black font-bold text-lg">✕</button>
              </div>

              {/* 顧客資料與問卷紀錄 */}
              <div className="space-y-3 text-xs text-[#4A574B] bg-[#FAFBF9] p-4 rounded-xl border border-[#DCE4DC]">
              ...
              <div className="space-y-2 bg-amber-50/50 p-4 rounded-2xl border border-amber-200/60">
                <div className="flex justify-between items-center">
                  <label className="block text-xs font-bold text-[#2D3B2E]">
                    🔒 店家內部備註 <span className="text-[#7A8A7B] font-normal">(僅供內部記錄，顧客無法看見)</span>
                  </label>
                  <button
                ...
                <textarea
                  ...
                  className="w-full p-3 rounded-xl bg-white border border-[#DCE4DC] text-xs outline-none focus:ring-2 focus:ring-[#8BA88D] resize-none"
                />
              </div>
              ...
              <div className="space-y-3">
                ...
                {/* 順位一 */}
                <div className="flex items-center justify-between p-3 rounded-xl border border-[#DCE4DC] bg-white hover:border-[#5B7B5E] transition-all">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-[#5B7B5E] text-white text-xs font-medium rounded-lg hover:bg-[#4A664D]"
                  >
                    定案以此時間
                  </button>
                </div>
              ...
              <div className="pt-2 border-t border-[#E2EBE2] flex justify-between">
                <button
